        ...
        <Reveal>
          <div className="mb-12">
            <div className="flex items-baseline gap-4 mb-4">
              ...
              <a href="/intake" className="text-sm text-l-text-secondary hover:text-l-text transition-colors nav-link">
                Intake →
              </a>
            </div>
        ...
        <Reveal delay={200}>
          <div className="card-panel p-0 overflow-hidden">
            ...
            <div className="flex gap-0 p-3 overflow-x-auto">
              {/* Backlog */}
              <div className="min-w-[200px] flex-1 border-r border-l-border px-2 board-column">
                ...
                <div className="space-y-1.5">
                  {issues.map((i, idx) => (
                    <div key={i.id} className="issue-card rounded-md bg-l-bg-tertiary/50 p-2 border border-l-border/50" style={{ animation: `fadeIn 0.3s ease ${0.3 + idx * 0.05}s both` }}>
              ...
              <div className="min-w-[200px] flex-1 border-r border-l-border px-2 board-column">
                ...
                <div className="space-y-1.5">
                  {todoIssues.map((i, idx) => (
                    <div key={i.id} className="issue-card rounded-md bg-l-bg-tertiary/50 p-2 border border-l-border/50" style={{ animation: `fadeIn 0.3s ease ${0.4 + idx * 0.05}s both` }}>
                      <div className="text-[10px] text-l-text-quaternary font-mono mb-0.5">{i.id}</div>
                      <div className="text-xs text-l-text-secondary leading-snug truncate">{i.title}</div>
              ...
              <div className="min-w-[200px] flex-1 border-r border-l-border px-2 board-column">
                ...
                <div className="space-y-1.5">
                  {inProgress.map((i, idx) => (
                    <div key={i.id} className="issue-card rounded-md bg-l-bg-tertiary/50 p-2 border border-l-border/50" style={{ animation: `fadeIn 0.3s ease ${0.5 + idx * 0.05}s both` }}>
                      <div className="text-[10px] text-l-text-quaternary font-mono mb-0.5">{i.id}</div>
                      <div className="text-xs text-l-text-secondary leading-snug truncate">{i.title}</div>
                      {i.labels && (
                        <div className="flex gap-1 mt-1">
              ...
              {/* Done */}
              <div className="min-w-[200px] flex-1 px-2 board-column">
                <div className="flex items-center justify-between mb-3 px-1">
                  <div className="flex items-center gap-2">
                ...
                <div className="space-y-1.5">
                  {done.map((i, idx) => (
                    <div key={i.id} className="issue-card rounded-md bg-l-bg-tertiary/50 p-2 border border-l-border/50" style={{ animation: `fadeIn 0.3s ease ${0.6 + idx * 0.05}s both` }}>
                      <div className="text-[10px] text-l-text-quaternary font-mono mb-0.5">{i.id}</div>
                      <div className="text-xs text-l-text-secondary leading-snug truncate">{i.title}</div>
